...
function createDatabase() {
  ...
  const defaultTemplate = db
    .prepare("SELECT id FROM show_templates WHERE is_default = 1")
    .get();
  if (!defaultTemplate) {
    const insertTemplate = db.prepare(
      "INSERT INTO show_templates (name, is_default, created_date) VALUES (?, 1, ?)",
    );
    ...
    const defaultSegments = [
      ...
      {
        name: "Extended host bit/segment",
        duration: 11,
        order: 6,
        type: "Extended Host Bit",
      },
      {
        name: "Headliner intro",
        duration: 1,
        order: 7,
        type: "Headliner Intro",
      },
      { name: "Headliner set", duration: 15, order: 8, type: "Headliner Set" },
      {
        name: "Show close/announcements",
        duration: 2,
        order: 9,
        type: "Show Close",
      },
...
ipcMain.handle("add-comedian", (event, comedian) => {
  ...
  const stmt = db.prepare(
    "INSERT INTO comedians (name, audio_file_path, default_duration) VALUES (?, ?, ?)",
  );
  const result = stmt.run(
    comedian.name,
    comedian.audioFilePath || null,
    comedian.defaultDuration,
  );
...
ipcMain.handle("update-comedian", (event, id, comedian) => {
  ...
  const stmt = db.prepare(
    "UPDATE comedians SET name = ?, audio_file_path = ?, default_duration = ? WHERE id = ?",
  );
  stmt.run(
    comedian.name,
    comedian.audioFilePath || null,
...
ipcMain.handle("add-template", (event, template) => {
  ...
  const stmt = db.prepare(
    "INSERT INTO templates (name, audio_file_path, default_duration, type) VALUES (?, ?, ?, ?)",
  );
  const result = stmt.run(
    template.name,
    template.audioFilePath || null,
...
ipcMain.handle("update-template", (event, id, template) => {
  ...
  const stmt = db.prepare(
    "UPDATE templates SET name = ?, audio_file_path = ?, default_duration = ?, type = ? WHERE id = ?",
  );
  stmt.run(
    template.name,
    template.audioFilePath || null,
...
ipcMain.handle("get-default-show-template", () => {
  const template = db
    .prepare("SELECT * FROM show_templates WHERE is_default = 1")
    .get();
  if (!template) return null;

  const segments = db
    .prepare(
      "SELECT * FROM show_template_segments WHERE show_template_id = ? ORDER BY order_index",
    )
    .all(template.id);
...
ipcMain.handle("get-shows", () => {
  const shows = db
    .prepare("SELECT * FROM shows ORDER BY created_date DESC")
    .all();
...
ipcMain.handle("get-show", (event, id) => {
  ...
  const segments = db
    .prepare("SELECT * FROM segments WHERE show_id = ? ORDER BY order_index")
    .all(id);
...
ipcMain.handle("save-show", (event, show) => {
  try {
    ...
    const transaction = db.transaction(() => {
      const stmt = db.prepare(
        "INSERT INTO shows (name, created_date, total_duration) VALUES (?, ?, ?)",
      );
      const result = stmt.run(
        show.name,
        new Date().toISOString(),
...
ipcMain.handle("update-show", (event, id, show) => {
  try {
    ...
    const transaction = db.transaction(() => {
      db.prepare(
        "UPDATE shows SET name = ?, total_duration = ? WHERE id = ?",
      ).run(show.name, totalDuration, id);
